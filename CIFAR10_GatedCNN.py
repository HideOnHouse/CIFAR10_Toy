...
def train(model, epoch=32, device='cuda'):
    ...
    for e in range(1, epoch + 1):
        with tqdm(train_loader, ascii=False) as t:
    ...
    with open(f"CIFAR_10_Gated.pt", 'wb') as f:
        torch.save(model, f)
...
def evaluate(model_path=None, device='cuda', show=False):
    ...
    if model_path is None:
        with open("CIFAR_10_Gated.pt", 'rb') as f:
            model = torch.load(f)
    ...
    criterion = nn.CrossEntropyLoss()
    with torch.no_grad():
        with tqdm(test_loader, ascii=False) as t:
            count = 0
            for image, label in t:
                image = image.to(device)
                label = label.to(device)
